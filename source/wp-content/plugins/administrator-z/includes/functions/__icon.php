...
function adminz_get_list_icons() {
    $options = ['' => __('Select') . ' adminz ' . strtolower(' ' . __('Icon'))];

    // adminz icon
    foreach (adminz_get_settings('Icons', 'icons') as $icon => $name) {
        $options[$icon] = "Adminz $icon";
    }

    // dashicons
    foreach (adminz_get_settings('Icons', 'dashicons') as $icon => $name) {
        $options[$icon] = "$icon";
    }
...
function adminz_get_list_dashicons() {
    /* const dashicons = document.querySelectorAll("li.dashicons");
	dashicons.forEach(li=>{
