...
require_once(ADMINZ_DIR . 'includes/shortcodes/inc/flatsome-custom-blogs-hooks.php');
$___ = new \Adminz\Helper\FlatsomeELement;
$___->shortcode_name = 'adminz_custom_blogs';
$___->shortcode_title = 'Custom blog posts';
...
// check file exists
if (!file_exists($flatsome_dir . '/commons/relay.php')) {
    return;
}
require_once(get_template_directory() . '/inc/builder/helpers.php');


// ------------ start copied from wp-content/themes/flatsome/inc/builder/shortcodes/blog_posts.php ---------------
$repeater_columns = '4';
    ...
    'style_options' => array(
        ...
        'options' => array(
            'style' => array(
                ...
                'heading' => __('Style'),
                'default' => '',
                'options' => require($flatsome_dir . '/values/box-layouts.php')
            )
        ),
    ),
    'layout_options' => require($flatsome_dir . '/commons/repeater-options.php'),
    'relay_options' => require($flatsome_dir . '/commons/relay.php'),
    ...
    'post_title_options' => array(
        ...
        'options' => array(
            'title_size' => array(
                ...
                'heading' => 'Title Size',
                'default' => '',
                'options' => require($flatsome_dir . '/values/sizes.php')
            ),
            'title_style' => array(
                'type' => 'radio-buttons',
                'heading' => 'Title Style',
...
$adminz_group = [
    'adminz' => [
        ...
        'options' => [
            ...
            'adminz_terms_excluded' => [
                'type' => 'select',
                'heading' => 'Terms excluded',
                'default' => '',
                'config' => array(
                    'multiple' => true,
                    'placeholder' => 'Select..',
...
foreach ($post_types as $post_type => $args) {
    if (is_post_type_viewable($args)) {
        // post type
        $adminz_group['adminz']['options']['adminz_post_type']['config']['options'][$post_type] = $args->labels->name;
        // term_taxonomy_included and excluded
        $taxonomies = get_object_taxonomies($post_type);
        foreach ($taxonomies as $taxonomy) {
            $terms = get_terms($taxonomy);
            foreach ($terms as $key => $value) {
                //
                $option_key = $value->term_id;
                $option_label = html_entity_decode("[$taxonomy] $value->name");
                //
                $adminz_group['adminz']['options']['adminz_terms_included']['options'][$option_key] = $option_label;
                $adminz_group['adminz']['options']['adminz_terms_excluded']['options'][$option_key] = $option_label;
            }
        }
        //
        $adminz_group['adminz']['options']['adminz_ids']['config']['postSelect']['post_type'][] = $post_type;
    }
}
$options = array_merge($adminz_group, $options);
...
$___->shortcode_callback = function ($atts, $content = null, $tag = '') {
    // Copy toàn bộ source 
    // Thêm các tham số custom
    // xử lý tham số custom
    // Thêm paged và xử lý bằng javascript

    // enqueue
    wp_enqueue_script(
        'adminz_custom_blogs_js',
        ADMINZ_DIR_URL . '/assets/js/adminz-flatsome-custom-blogs.js',
        array('jquery'),
        ADMINZ_VERSION,
        true
    );
    wp_enqueue_style(
        'adminz_custom_blogs_css',
        ADMINZ_DIR_URL . '/assets/css/flatsome/flatsome-custom-blogs.css',
        array(),
        ADMINZ_VERSION
    );
    ...
    extract($atts = shortcode_atts(array(
        ...
        // ================ start custom =========================
        'adminz_post_type' => 'post',
        'adminz_terms_included' => '',
        'adminz_terms_excluded' => '',
        'adminz_ids' => '',
        'adminz_paged_style' => '',
        'adminz_loadmore' => 'Load more',
        // ================ end custom ===========================

    ), $atts));

    // ================ start custom =========================
    $atts = apply_filters('adminz_fl_custom_blog_posts_atts', $atts);
    extract($atts); // extract again
    // ================ end custom ===========================        
    ...
    // ================ start custom =========================
    $args = apply_filters('adminz_fl_custom_blog_posts_args', $args, $atts);
    // ================ end custom ===========================
    ...
    // ================ start custom =========================
    $adminz_classes = [
        defined('WP_DEBUG_DISPLAY') && WP_DEBUG_DISPLAY && current_user_can('administrator') ? 'wp_debug_display' : '',
        'type_' . $atts['type'],
        'post_type_' . $atts['adminz_post_type'],
        'paged_style_' . $atts['adminz_paged_style'],
    ];
    $adminz_classes = implode(' ', $adminz_classes);
    echo "<div class='adminz_custom_blogs $adminz_classes'>";
    echo "<div class='inner_wrap'>";
    // ================ end custom ===========================
                    ...
                    <?php if (has_post_thumbnail()) { ?>
                        <div class="box-image" <?php echo get_shortcode_inline_css($css_args_img); ?>>
                            <div class="<?php echo esc_attr($classes_image); ?>" <?php echo get_shortcode_inline_css($css_image_height); ?>>
                                ...
                                $_link = get_permalink();
                                $_class = 'plain';
                                if ($video_url = get_post_meta(get_the_ID(), '_adminz_video_url', true)) {
                                    $_link = $video_url;
                                    $_class .= ' open-video';
                                }
                                ?>

                                <a href="<?php echo esc_url($_link); ?>" class="<?php echo esc_attr($_class); ?>" aria-label="<?php echo esc_attr(the_title()); ?>">
...
<?php
    ...
    do_action('after_adminz_fl_custom_blog_posts', $repeater, $atts, $recentPosts);
    echo '</div>'; // end of adminz_custom_blogs
    // ================ end custom ===========================
